...
class FreeEnergyPlotter(object):
    ...
    def plot_bar_plot(self,legend=('experimental', 'computed'), colors=[sns.xkcd_rgb["pale red"], sns.xkcd_rgb["denim blue"]]):
        ...
        rects1 = ax.bar(ind,  self.dataseries1[:,0], width, color=colors[0])
        rects2 = ax.bar(ind + width,  self.dataseries2[:,0], width, color=colors[1])
    ...
    def plot_scatter_plot(self, xlabel=r'experimental $\Delta \Delta G$ in [kcal/mol]', ylabel=r'computed $\Delta \Delta G$ in [kcal/mol]', color=sns.xkcd_rgb["pale red"]):
        ...
        plt.errorbar(self.dataseries1[:,0], self.dataseries2[:,0], yerr=self.dataseries2[:,1], color = sns.xkcd_rgb["pale red"], linewidth=0, elinewidth=1, marker = 'o')
