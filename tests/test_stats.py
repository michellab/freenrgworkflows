...
def test_property_errors(stats, boundaries):
    ...
    stats.confidence_interval=boundaries
    assert(stats.R_error == [0.8682078950591153, 0.9338724006518556])
    assert(stats.tau_error == [1.0, 1.0])
    assert(stats.mue_error == [0.3562336854267332, 0.5309578343443963])
    assert(stats.R2_error == [0.7537849490429798, 0.8721176606992599])
